...
function handleSave() {
  const teacherNo = form.value.teacherNo.trim()
  const teacherName = form.value.teacherName.trim()
  const gender = form.value.gender

  if (!teacherNo) {
    showToast('请输入教师工号', 'error')
    return
  }
  if (!teacherName) {
    showToast('请输入教师姓名', 'error')
    return
  }
  if (!gender) {
  ...
  const payload = { teacherNo, teacherName, gender }
  const successMessage = editingId.value ? '修改教师成功' : '新增教师成功'
  const request = editingId.value
    ? updateTeacher(editingId.value, payload)
    : addTeacher(payload)
...
async function handleDelete(row) {
  const confirmed = await confirmAction({
    title: '删除教师',
    message: `确认删除「${row.teacherName}」？删除后不可恢复。`,
    confirmText: '删除',
    type: 'danger',
  })
  if (!confirmed) return
...
<template>
  <section class="page-section">
    <div class="page-title">
      <h2>教师管理</h2>
      <button type="button" class="primary" @click="openAdd">新增教师</button>
    </div>
      ...
      <div class="panel-body">
        <table>
          ...
          <tbody>
            ...
            <tr v-for="(row, idx) in tableData" :key="row.teacherId">
